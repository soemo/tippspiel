...
describe Tips::Save do
  ...
  let!(:game1) { create(:game, start_at: 1.day.from_now, finished: true) }
  let!(:game2) { create(:game, start_at: 1.day.from_now, finished: true) }
  let!(:game3) { create(:game, start_at: 1.second.ago, finished: true) }
  ...
  context 'if tip_params and user present?' do
    it 'save tips' do
      ...
      expect(tip3.team1_goals).to be_nil # dieser Tipp durfte nicht mehr abgegeben werden
      expect(tip3.team2_goals).to be_nil # dieser Tipp durfte nicht mehr abgegeben werden
    end
  end

  context 'if tip_params and user not present?' do
    it 'saves no tips' do
      expect(Tips::FromUser).not_to receive(:call)
