...
describe TipsController do
  ...
  describe '#save_tips' do
    ...
    it 'saves only tips for games start in the future' do
      ...
      game.update_column(:start_at, 1.second.from_now) # Spielstart in der Zukunft

      # update erlaubt
      post :save_tips, params: { tips: { tip.id.to_s => { 'team1_goals' => '9', 'team2_goals' => '9' } } }
      ...
      game.update_attribute(:start_at, Time.zone.now) # Spiel startet genau jetzt

      # update NICHT erlaubt
      post :save_tips, params: { tips: { tip.id.to_s => { 'team1_goals' => '0', 'team2_goals' => '3' } } }
      expect(response).to redirect_to root_path
