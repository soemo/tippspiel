...
module Admin
  class ResultImportsController < Admin::BaseController
    def create # rubocop:disable Metrics/MethodLength,Metrics/AbcSize -- error handling requires multiple rescue clauses; extracting would obscure the control flow
      ...
      if @result.changes?
        begin
          ...
          Rails.logger.error("ResultImportsController: mail delivery failed — #{e.class}: #{e.message}")
